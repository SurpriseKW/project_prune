...
def train():
    ...
    trainLoader, _ = get_dataloader.get_stl10(batch_size, num_workers)

    model_folder = sys.path[0] + os.sep + 'vgg16_cifar10_CAWR_' + time.strftime('%Y%m%d_%H%M%S', time.localtime())
    ...
    torch.save(model, model_folder + os.sep + 'vgg16.pth')
